  ...
  return (
    <div className="mx-auto max-w-xl">
      <GlassPanel className="p-6 text-center">
        <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full border-2 border-nexus/50 bg-nexus/10 font-display text-2xl font-bold text-nexus glow-accent">
          KP
        </div>
        ...
        <p className="mt-1 flex items-center justify-center gap-2 font-mono text-xs text-muted-foreground">
          <StatusDot tone="success" /> ACTIVE DUTY — NEXUS OPERATIONS
        </p>
        <div className="mt-6 grid grid-cols-3 gap-3">
          {STATS.map((stat) => (
            <div key={stat.label} className="rounded-xl border border-white/8 bg-white/4 p-3">
              <p className="font-display text-lg text-nexus tabular-nums">{stat.value}</p>
